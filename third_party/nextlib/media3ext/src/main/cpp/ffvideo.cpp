...
JniContext *createVideoContext(JNIEnv *env,
    ...
    if (rawWidth > 0) {
        codecContext->width = rawWidth;
        codecContext->coded_width = rawWidth;
    }
    if (rawHeight > 0) {
        codecContext->height = rawHeight;
        codecContext->coded_height = rawHeight;
    }
    if (rawBitrate > 0) {
        codecContext->bit_rate = rawBitrate;
    }
...
Java_io_github_anilbeesetti_nextlib_media3ext_ffdecoder_FfmpegVideoDecoder_ffmpegInitialize(JNIEnv *env,
    ...
    return (jlong) createVideoContext(env, codec, extra_data, threads, raw_width, raw_height, raw_bitrate);
}
...
Java_io_github_anilbeesetti_nextlib_media3ext_ffdecoder_FfmpegVideoDecoder_ffmpegSendPacket(JNIEnv *env,
    ...
    if (encoded_data == nullptr) {
        // Flush.
        int result = avcodec_send_packet(avContext, nullptr);
        if (result) {
    ...
    if (inputBuffer == nullptr && length > 0) {
        LOGE("Encoded data is not a direct buffer.");
        return VIDEO_DECODER_ERROR_OTHER;
    }

    AVPacket *packet = av_packet_alloc();
    if (packet == nullptr) {
        LOGE("video_decoder_send_packet: av_packet_alloc failed");
        return VIDEO_DECODER_ERROR_OTHER;
    }

    if (length > 0) {
        // Create a refcounted packet (with padding) so codecs can keep references as needed.
        int allocResult = av_new_packet(packet, length);
        if (allocResult < 0) {
            logError("av_new_packet", allocResult);
            av_packet_free(&packet);
            return VIDEO_DECODER_ERROR_OTHER;
        }
        memcpy(packet->data, inputBuffer, length);
        packet->size = length;
    } else {
        packet->data = nullptr;
        packet->size = 0;
    ...
    av_packet_free(&packet);
    if (result) {
        logError("video avcodec_send_packet", result);
        LOGE(
                "send_packet details: codec_id=%d w=%d h=%d extradata_size=%d packet_size=%d",
                avContext ? avContext->codec_id : -1,
                avContext ? avContext->width : -1,
                avContext ? avContext->height : -1,
                avContext ? avContext->extradata_size : -1,
                length);

        if (result == AVERROR(EAGAIN)) {
            // need read frame
            return VIDEO_DECODER_ERROR_READ_FRAME;
        } else if (result == AVERROR_INVALIDDATA || result == AVERROR(EINVAL) || result == AVERROR(EPERM)) {
            // Treat as a recoverable decode error: flush and skip this packet.
