...
final class FfmpegVideoDecoder extends
    ...
    public FfmpegVideoDecoder(int numInputBuffers, int numOutputBuffers, int initialInputBufferSize, int threads, Format format) throws FfmpegDecoderException {
        ...
        nativeContext =
                ffmpegInitialize(
                        ...
                        extraData,
                        threads,
                        format.width != Format.NO_VALUE ? format.width : 0,
                        format.height != Format.NO_VALUE ? format.height : 0,
    ...
    private static byte[] getExtraData(String mimeType, List<byte[]> initializationData) {
        ...
        switch (mimeType) {
            ...
            case MimeTypes.VIDEO_VC1,
                    WmvMimeTypes.VIDEO_X_MS_WMV3,
                    WmvMimeTypes.VIDEO_X_MS_WMV2,
                    WmvMimeTypes.VIDEO_X_MS_MSMPEG4V3 -> {
                return initializationData.get(0);
            }
            default -> {
    ...
    protected FfmpegDecoderException decode(DecoderInputBuffer inputBuffer, VideoDecoderOutputBuffer outputBuffer, boolean reset) {
        ...
        // enqueue origin data
        int sendPacketResult = ffmpegSendPacket(nativeContext, packetData, inputSize, inputBuffer.timeUs);
