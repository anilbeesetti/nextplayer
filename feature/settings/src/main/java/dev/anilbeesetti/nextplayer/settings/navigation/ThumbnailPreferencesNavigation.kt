...
import dev.anilbeesetti.nextplayer.settings.screens.thumbnail.ThumbnailPreferencesRoute
...
fun EntryProviderScope<NavKey>.thumbnailPreferencesEntry(onNavigateUp: () -> Unit) {
    entry<ThumbnailPreferencesRoute> {
        ThumbnailPreferencesRoute(output = ThumbnailPreferencesViewModel.Output(navigateUp = onNavigateUp))
    }
}
