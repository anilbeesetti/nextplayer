...
class TopLevelNavigationTest {
    @Test
    fun playlistsIsASeparateMiddleTopLevelDestination() {
        assertEquals(
            listOf(
    ...
    fun switchingTabsPreservesPlaylistDetailStack() {
        ...
        playlistStack += PlaylistDetailRoute(playlistId = 42)
        state.switchTo(TopLevelDestination.MEDIA.route)
        state.switchTo(TopLevelDestination.PLAYLISTS.route)

        assertEquals(
            listOf(TopLevelDestination.PLAYLISTS.route, PlaylistDetailRoute(42)),
            state.currentStack,
        )
    }
}
